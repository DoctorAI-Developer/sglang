...
class TestServerArgsOwnership(_IsolatedServerArgs):
    ...
    def test_legacy_setter_publishes_into_context(self):
        # Identity (not equality) is the contract: the slot holds the very
        # object that was published, and publish resolves a record on the way
        # in, so the object has to be one.
