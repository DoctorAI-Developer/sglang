...
# what it saw. This predates the declaration work -- the speculative hooks run
# at dispatch step 58, the first `get_model_config()` at step 18 -- and it is
# pinned rather than fixed because fixing it means moving one of those two.
# The consequence is real: the alias pass upper-cases the algorithm, so a
# `--speculative-algorithm dspark` launch leaves the cached configuration at
# `"dspark"` while `checkpoint_bundles_dspark_draft` compares against the
# upper-case literal, and the DSpark draft architecture is not selected.
