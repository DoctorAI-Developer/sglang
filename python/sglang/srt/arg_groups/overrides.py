...
def declare_resolution(server_args: Any, source: str, **fields: Any) -> None:
    ...
    resolver's successors seeing the value where they read the field
    directly, so routing a write through here changes nothing about ordering.
    ...
    """
    # Non-dataclass fixtures declare no fields (mirrors the validate_declarations
    # escape); only a real ServerArgs is checked.
    ...
    for name, value in fields.items():
        # A plain assignment, so the read-only guard applies: this is a
        # resolution-time mechanism, and a caller reaching for it after
        # materialization wants declare_late_resolution instead.
...
def declare_late_resolution(server_args: Any, source: str, **fields: Any) -> None:
    ...
    # Also a declaration: this is resolution, just staged after `__post_init__`
    # because it needs a tokenizer or an adapter load. The projection reads the
    # declarations, so a late write that only reached the fields would publish
    # bags describing the *unresolved* value -- an `auto` parser that detection
    # has already replaced, LoRA paths that normalization has already parsed.
    stash = getattr(server_args, "_resolved_overrides", None)
    if stash is None:
        stash = []
        object.__setattr__(server_args, "_resolved_overrides", stash)
...
def declare_direct_writes(
    ...
    Rebinding is what the diff sees. A plugin that mutates a value in place --
    appending to a list, setting a key on a nested config -- leaves nothing to
    compare, and that write stays invisible.
    """
    before = {
